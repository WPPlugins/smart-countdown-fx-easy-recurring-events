	...
	private static function addEasyRecurrenceChain( &$imported, $unit, $duration, $countdown_to_end, $recurrence_config = array(), $base_date = null ) {
		...
		try {
			if( $unit == 'hour' ) {
				...
				$each_hours = $recurrence_config['interval'];
					
				// we have to pull events from 24 hours ago to support maximum event duration. 
				// Starting from -24h is safe because all supported recurrence intervals guarantee 
				// that one of the events takes place at "base_date" time every day.
				// we stop pulling  in future events at 24 hours + interval from the base date
				for ( $delta = -24; $delta <= 24 + $each_hours; $delta += $each_hours ) {
					$tmp = clone ( $base_date );
					$diff = $delta >= 0 ? '+' . $delta : $delta;
					$tmp->modify( $diff . ' hour' );
